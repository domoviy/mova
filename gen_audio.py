...
# Було 3 → піднято до 5 (перевірки цілісності стали строгими, транзитним
# мережевим збоям треба більше спроб) → повернуто до 3: на практиці
# більшість реальних збоїв — НЕ транзитні мережеві глюки, а стійка,
# детермінована розбіжність у конкретному тексті (той самий результат
# на кожній спробі підряд, часто ще й повторюється на наступному запуску
# скрипта) — зайві спроби для такого випадку лише витрачають час, не
# змінюючи результат. 3 спроби з експоненційною паузою й далі достатньо
...
# '=' — той самий принцип, що '&', але навпаки за напрямом помилки:
# символ MAE озвучуватись (рівняння на кшталт "Assets = Liabilities +
# Stockholders' Equity" реально мають прозвучати як "Assets EQUALS
# Liabilities..."), просто наш токенізатор його не рахував, тоді як
# Edge TTS таки вимовляє — звідси "отримано БІЛЬШЕ, ніж очікує текст".
# На відміну від '+' (див. clean_text — те чисто нотаційний роздільник,
# тому просто прибирається), тут підставляємо слово, а не видаляємо.
EQUALS_WORDS = {"de": "ist gleich", "en": "equals", "uk": "дорівнює", "ru": "равно"}
...
def clean_text(text, lang=None):
    ...
    if '=' in text:
        eq_word = EQUALS_WORDS.get(lang, 'equals')
        text = re.sub(r'\s*=\s*', ' ' + eq_word + ' ', text)

    # '+' — той самий клас проблеми, що '&' щойно вище, лише в ІНШИЙ бік:
    # Edge TTS, судячи з усього, озвучує сам символ як слово "Plus" —
    # WordBoundary-подій виявляється БІЛЬШЕ, ніж очікує токенізатор (не
    # менше, як у випадку з '&'), напр. "sich schützen (vor + Dat.)" —
    # очікуємо 4 токени, Edge дає 5. У базі курсу '+' — ЛИШЕ граматична
    # позначка (напр. "vor + Dat." = "прийменник vor керує давальним
    # відмінком"), а не математичний плюс, тож на відміну від '&' тут
    # не підставляємо слово — просто прибираємо символ повністю.
    # '+' одразу після відкриваючої дужки, без пробілу перед ним, як
    # "(+ дат. пад.)" (uk/ru/en-переклади граматичних приміток; у
    # німецькому "der Umgang mit + Dativ" такого немає — там '+' і так
    # оточений пробілами з обох боків, загальне правило нижче впорається
    # саме). Загальне правило \s*\+\s* → ' ' тут дало б "( дат. пад.)" —
    # вставило пробіл МІЖ дужкою і текстом, якого не було в оригіналі:
    # дужка "відокремлюється" від решти, і Edge TTS, судячи з усього,
    # озвучує цю відокремлену дужку як фантомне зайве слово (звідси
    # "отримано 5, очікує 4"). Тому цей випадок — ОКРЕМО, ДО загального
    # правила: "(+ " → "(" без жодного зайвого пробілу.
    if '(+' in text:
...
def _tokenize_like_frontend(text):
    ...
    ⚠️ Один нюанс, який регекс сам по собі НЕ покриває: EN DASH ('–',
    U+2013) справді не матчиться (це не '-' символ класу, а інший
    юнікод-символ) — але ЗВИЧАЙНИЙ ASCII-дефіс ('-', U+002D), коли він
    стоїть САМ ПО СОБІ між пробілами як пунктуаційна пауза ("ist -
    diese"), матчиться, бо '-' явно є символом класу [\\w'-]+ — і стає
    ОКРЕМИМ токеном-"словом" з одного символу. Edge TTS такий
    самостійний дефіс так само не озвучує (не створює під нього
    WordBoundary-подію) — тому цей "фантомний" токен систематично
    зсував очікувану кількість на +1 і викликав хибне 'обірвано' на
    картках з таким дефісом (dlg_031/034/039/040_a8 — 'ist -
    diese'/'Lebenslauf - das' і т.п.), попри повністю справний файл.
    Фільтруємо ПІСЛЯ findall: токен лишається, лише якщо після
    видалення дефісів з нього щось лишилось (реальні слова з дефісом,
    внутрішнім чи кінцевим, як 'EDV-' чи 'CO2-Ausstoßes', проходять
    без змін — там після strip('-') лишається текст)."""
    tokens = re.findall(r"[\w'-]+", text, re.UNICODE)
    return [t for t in tokens if t.strip('-')]


def _norm_word(w):
    """Нормалізація слова для ЗМІСТОВНОГО порівняння (не для підрахунку
    токенів — токенізація лишається в _tokenize_like_frontend). Прибирає
    все, крім літер/цифр/апострофа/дефіса, і додатково — це критично —
    ОБРІЗАЄ дефіс НА КРАЯХ слова ('-' на початку чи в кінці), лишаючи
    лише ВНУТРІШНІЙ (з'єднувальний, як у "CO2-Ausstoßes"). Причина:
    в базі трапляється еліптична німецька конструкція "EDV- und
    Sprachkenntnissen" (спільний суфікс для двох слів) — токен "EDV-"
    із дефісом на кінці лишається графічним прийомом, який НЕ
    вимовляється; WordBoundary від Edge TTS для цього слова повертає
    "EDV" (без дефіса). Якщо порівнювати "edv-" з "edv" напряму — вони
    НЕ рівні попри те, що це те саме слово, і раніше саме це давало
    хибне "обірвано" навіть при точному збігу кількості токенів
    (форум-картка forum_008: 28 отримано = 28 очікується, і все одно
    падало)."""
...
def _looks_truncated(cleaned_text, boundary_words):
    ...
    ⚠️ ДРУГА ІТЕРАЦІЯ цієї перевірки (перша — м'який поріг ratio≥0.85 —
    пропускала втрату ОДНОГО слова ПОСЕРЕДИНІ фрази; друга — "БУДЬ-ЯКА
    розбіжність послідовності = обрив", нуль толерантності) виявилась
    ЗАНАДТО суворою і на РЕАЛЬНИХ даних: живий прогін показав 100%-но
    ВІДТВОРЮВАНІ (не випадкові — однакова розбіжність на кожній з 5
    спроб) "хибні обриви". Причина — Edge TTS сегментує WordBoundary-
    події НЕ завжди 1:1 з наївною токенізацією за пробілами/дефісами:
      • "75 km", "800 Kilometern" (число + одиниця виміру) — Edge інколи
        звітує як МЕНШЕ подій, ніж окремих написаних токенів (об'єднує).
      • Складне слово може навпаки розбитись на БІЛЬШЕ подій, ніж один
        токен (протилежний напрямок).
      • "EDV-" (дефіс на КРАЮ токена, еліптична конструкція) — та сама
        кількість токенів, але дефіс на кінці "з'їдається" при вимові
        (виправлено окремо в _norm_word).

    Тому тепер — НЕ "будь-яка розбіжність = обрив", а ЗМІСТОВНЕ
    порівняння кожного НЕ-збіжного відрізка вирівнювання
    (difflib.SequenceMatcher): якщо ОБ'ЄДНАНИЙ (конкатенований) нормалізо-
    ваний текст очікуваних токенів на цьому відрізку ДОСЛІВНО збігається
    з об'єднаним текстом реально отриманих токенів — це просто ІНША
    сегментація Edge TTS (число+одиниця злилися в одну подію, чи
    навпаки розбились), а НЕ втрата змісту, і не карається. А ось якщо
    об'єднаний текст РІЗНИЙ (тобто слово справді зникло чи спотворилось,
    а не просто інакше розбите на токени) — це вже реальний обрив.

    Це вирішує головну задачу (ловить слово, загублене ПОСЕРЕДИНІ фрази
    — концентрація об'єднаного тексту миттєво покаже нестачу символів),
    залишаючись стійким до легітимних відмінностей сегментації Edge."""
    ...
    expected_norm = [_norm_word(w) for w in expected]
    got_norm = [_norm_word(w) for w in boundary_words]
...
def _looks_timing_desynced(words, audio_duration_sec):
    ...
    ⚠️ АСИМЕТРИЧНА толерантність — напрями РІЗНІ за небезпечністю:

    1) audio_duration < last_end (аудіо КОРОТШЕ за заявлений таймінг) —
       НЕБЕЗПЕЧНИЙ випадок: WordBoundary-канал (текстові повідомлення)
       ...
       метаданих збігаються з текстом!). Тут тримаємо ЖОРСТКИЙ поріг.

    2) audio_duration > last_end (аудіо ДОВШЕ за заявлений таймінг) —
       БЕЗПЕЧНИЙ, ЗВИЧНИЙ випадок: Edge TTS майже завжди додає невелику
       технічну паузу/затухання ПІСЛЯ останнього слова, яка не потрапляє
       у WordBoundary-таймінг. Емпірично (прогін forumsbeitrag,
       de-DE-KatjaNeural/FlorianMultilingualNeural/SeraphinaMultilingualNeural)
       цей "хвіст" стабільно ~0.85-0.9с на звичайних репліках і трохи
       більший (~1.0-1.3с) на дуже коротких (single-word) — причому
       АБСОЛЮТНО ІДЕНТИЧНИЙ на кожній із 5 повторних спроб одного файлу,
       що виключає випадковий мережевий глюк і підтверджує: це штатна
       поведінка рушія, не обрив. У застосунку це виглядає щонайбільше
       як коротка пауза перед кнопкою "Далі" — не як обрив слова
       посередині. Тримаємо тут ЩЕДРИЙ поріг, щоб не відхиляти валідні
       файли пачками.
    """
    ...
    if gap < 0:
        # Аудіо коротше за таймінг — потенційний обрив, поріг жорсткий.
        tolerance = max(0.6, 0.05 * audio_duration_sec)
    else:
        # Аудіо довше за таймінг — типова технічна пауза в кінці,
        # поріг щедрий (fixed 2.0с + запас на дуже короткі репліки).
        tolerance = max(2.0, 0.15 * audio_duration_sec)
        ...
        for attempt in range(1, RETRY_ATTEMPTS + 1):
            try:
                ...
                if attempt < RETRY_ATTEMPTS:
                    # Експоненційне зростання (2с → 4с → 8с → …) обмежене
                    # стелею 4с — це був довільний вибір за замовчуванням,
                    # жодна зовнішня вимога такого зростання не диктує.
                    # За великої кількості файлів довший backoff помітно
                    # уповільнює загальний прогін без явної користі: якщо
                    # причина невдачі — короткий мережевий збій чи
                    # rate-limit, 4с зазвичай достатньо для відновлення;
                    # якщо ж причина інша (як-от справжній обрив
                    # з'єднання) — довша пауза все одно нічого не
                    # виправляє, лише витрачає час.
